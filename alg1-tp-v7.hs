...
estaRobertoCarlos rs | cantidadDeAmigos rs (usuarioConMasAmigos rs) > 2 = True
                     | otherwise = False
...
obtenerRedAmigosTotal rs us aux | iguales aux (rmvduplicates(aux ++ (obtenerRedAmigosParcial rs us))) = aux
                                | otherwise = obtenerRedAmigosTotal rs (obtenerRedAmigosParcial rs us) (rmvduplicates(aux ++ (obtenerRedAmigosParcial rs us)))
...
rmvduplicates :: Set Usuario -> Set Usuario
rmvduplicates [] = []
rmvduplicates (u:us) | elem u us = rmvduplicates us
                     | otherwise = u : rmvduplicates us
...
estaRobertoCarlos redA ~~~> True  (para no crear una red con un millon de amigos, testeamos diciendo que devuelva True si algun usuario tiene mas de 2 amigos)
 se testeo cambiando la linea de codigo 66 por esta:    estaRobertoCarlos rs | cantidadDeAmigos rs (usuarioConMasAmigos rs) > 2 = True
...
estaRobertoCarlos redB ~~~> False (este caso es pidiendo que tenga mas de un millon de amigos)
...
publicacionesDe redB usuario3 ~~~> [((3,"Pedro"),"Lorem Ipsum",[]),((3,"Pedro"),"dolor sit amet",[(2,"Natalia")]),((3,"Pedro"),"consectetur adipiscing elit",[(2,"Natalia"),(5,"Natalia")])]
